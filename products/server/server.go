...
// CreateProduct implementation for the rcp
...
// dbToProductPb transform dbProduct to productPB
...
// UpdateProduct implementation for the rcp
...
// DeleteProduct implementation for the rcp
func (*server) DeleteProduct(ctx context.Context, req *productpb.DeleteProductRequest) (*productpb.DeleteProductResponse, error) {
	...
	// return the product ID deleted
	return &productpb.DeleteProductResponse{
		ProductId: req.GetProductId(),
	}, nil
}

// ListProduct implementation for the rcp, using server streaming
func (*server) ListProduct(req *productpb.ListProductRequest, stream productpb.ProductService_ListProductServer) error {
	...
	if err != nil {
		return status.Errorf(
			codes.Internal,
			fmt.Sprintf("Internal error to get all products: %v", err),
		)
	}
...
func main() {
	...
	// connection to mongodb, put in a separate filder/file
	client := db.ConectarDB()
	if db.CheckConnection() == 0 {
		log.Fatal("Failed connection with the db")
		return
	}
	// get the collection to use
	collection = client.Database("goproducs").Collection("products")

	// create the server for the product service
